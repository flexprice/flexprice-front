...
const FeatureMultiSelect: FC<Props> = ({
	...
	return (
		<div className={cn('w-full')}>
			{resolvedLabel && <label className={cn('block text-sm font-medium text-foreground mb-1')}>{resolvedLabel}</label>}
			<MultiSelect
				options={options}
				...
				customDisplay={(count) => {
					...
					return (
						<div className='flex items-center justify-between w-full'>
							<span className='text-sm text-foreground font-normal'>{selectedSummary(count)}</span>
