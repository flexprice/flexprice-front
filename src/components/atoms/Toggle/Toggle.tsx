...
const Toggle: FC<Props> = ({ onChange, checked, description, error, label, title, className }) => {
	return (
		<div>
			<FormHeader title={title} variant='form-component-title' />
			<div className='flex items-start space-x-4 font-open-sans'>
				<Switch id='airplane-mode' checked={checked} onCheckedChange={onChange} className={cn(className, 'h-7 text-xs')} />
