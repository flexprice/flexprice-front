...
const SubscriptionsSection = ({ subscriptions, isLoading }: SubscriptionsSectionProps) => {
	...
	return (
		<Card className='bg-card border border-border rounded-xl p-6'>
			<h3 className='text-base font-medium text-foreground mb-4'>{t('subscriptions.title')}</h3>
			<div className='space-y-4'>
				{activeSubscriptions.map((subscription) => (
					<div key={subscription.id} className='border border-border rounded-lg p-4 hover:bg-muted transition-colors'>
						<div className='flex items-start justify-between mb-3'>
							<div>
								<h4 className='text-sm font-medium text-foreground'>{subscription.plan?.name || t('subscriptions.unknownPlan')}</h4>
								{subscription.plan?.description && (
									<p className='text-xs text-muted-foreground mt-0.5 line-clamp-1'>{subscription.plan.description}</p>
								)}
							</div>
