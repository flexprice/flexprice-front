		...
		<Card
			...
			<div className='p-6 space-y-4'>
				{activeSubscriptions.map((subscription) => (
					<div key={subscription.id} className='rounded-lg p-4 transition-colors' style={{ border: '1px solid var(--portal-border)' }}>
						<div className='flex items-start justify-between mb-3'>
							<div>
								<h4 className='text-sm font-medium' style={{ color: 'var(--portal-text-primary)' }}>
									{subscription.plan?.name || t('subscriptions.unknownPlan')}
								</h4>
								{subscription.plan?.description && (
									<p className='text-xs mt-0.5 line-clamp-1' style={{ color: 'var(--portal-text-secondary)' }}>
										{subscription.plan.description}
									</p>
								)}
							</div>
						...
						<div className='flex flex-wrap gap-4 text-xs' style={{ color: 'var(--portal-text-secondary)' }}>
