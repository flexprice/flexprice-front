					...
					<table className='w-full border-collapse'>
						<thead>
							<tr className='border-b border-line'>
								<th className='py-3 px-0 text-start text-sm font-medium text-content'>
									{t('invoices.details.lineItemsTable.subscription')}
								</th>
								{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
									<th className='py-3 px-4 text-end text-sm font-medium text-content'>
										{t('invoices.details.lineItemsTable.description')}
									</th>
								)}
								{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
									<th className='py-3 px-4 text-end text-sm font-medium text-content'>{t('invoices.details.lineItemsTable.interval')}</th>
								)}
								<th className='py-3 px-4 text-end text-sm font-medium text-content'>{t('invoices.details.lineItemsTable.quantity')}</th>
								<th className='py-3 px-0 text-end text-sm w-36 font-medium text-content'>{t('invoices.details.lineItemsTable.amount')}</th>
							</tr>
						</thead>
						<tbody>
							{filteredData?.map((item, index) => {
								return (
									<tr key={index} className='border-b border-line-subtle'>
										<td className='py-4 px-0 text-sm  text-content'>{item.display_name ?? na}</td>
										{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
											<td className='py-4 px-4 text-sm text-content-tertiary text-end'>
										...
										<td className='py-4 px-4 text-end text-sm text-content-tertiary'>{item.quantity ? item.quantity : na}</td>
										<td className='py-4 px-0 text-end w-36  text-sm text-content-tertiary'>
					...
					<div className='w-72 space-y-1'>
						...
						{subtotal !== undefined && subtotal !== null && Number(subtotal) !== 0 && (
							<div className='flex justify-between items-center py-1.5'>
								<span className='text-xs text-content-muted'>{t('invoices.details.lineItemsTable.subtotal')}</span>
								<span className='text-sm text-content font-medium'>{formatAmount(Number(subtotal), currency ?? '')}</span>
							</div>
						)}
						...
						{discount !== undefined && discount !== null && Number(discount) > 0 && (
							<div className='flex justify-between items-center py-1.5'>
								<span className='text-xs text-content-muted'>{t('invoices.details.lineItemsTable.discount')}</span>
								<span className='text-sm text-content-tertiary'>−{formatAmount(Number(discount), currency ?? '')}</span>
							</div>
						)}
						...
						{total_prepaid_credits_applied !== undefined &&
							...
							Number(total_prepaid_credits_applied) > 0 && (
								<div className='flex justify-between items-center py-1.5'>
									<span className='text-xs text-content-muted'>{t('invoices.details.lineItemsTable.prepaidCredits')}</span>
						...
						{total_tax !== undefined && total_tax !== null && Number(total_tax) !== 0 && (
							<div className='flex justify-between items-center py-1.5'>
								...
								<span className='text-sm text-content font-medium'>{formatAmount(Number(total_tax), currency ?? '')}</span>
							</div>
						)}

						{/* Net payable - always show, default to 0 if not provided */}
						<div className='flex justify-between items-center pt-3 mt-2 border-t border-line-subtle'>
							<div className='flex items-center gap-1.5'>
								<span className='text-sm text-content font-semibold'>{t('invoices.details.lineItemsTable.netPayable')}</span>
							...
							<span className='text-base text-content font-semibold'>{formatAmount(Number(amount_due ?? 0), currency ?? '')}</span>
						</div>

						{/* Amount paid - always show, default to 0 if not provided */}
						<div className='flex justify-between items-center py-1.5'>
							<span className='text-xs text-content-muted'>{t('invoices.details.lineItemsTable.amountPaid')}</span>
							<span className='text-sm text-content font-medium'>{formatAmount(Number(amount_paid ?? 0), currency ?? '')}</span>
						</div>
						...
						{overpaid_amount !== undefined && overpaid_amount !== null && Number(overpaid_amount) > 0 && (
							<div className='flex justify-between items-center py-1.5'>
								<span className='text-xs text-content-muted'>{t('invoices.details.lineItemsTable.overpaid')}</span>
								<span className='text-sm text-content font-medium'>{formatAmount(Number(overpaid_amount), currency ?? '')}</span>
							</div>
						)}
						...
						{((amount_remaining !== undefined && amount_remaining !== null && Number(amount_remaining) > 0) ||
							...
							<div className='flex justify-between items-center pt-3 mt-2 border-t border-line-subtle'>
								<span className='text-sm text-content font-semibold'>{t('invoices.details.lineItemsTable.remainingBalance')}</span>
								<span className='text-base text-content font-semibold'>
