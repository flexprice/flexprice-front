...
const TierBreakdown: FC<{ normalized: NormalizedPriceDisplay; hasOverrides?: boolean; t: TFunction<'common'> }> = ({
	...
	return (
		<div className='space-y-3'>
			<div className='font-medium border-b border-border pb-2 text-base text-foreground'>
				{title}
			...
			<div className='space-y-2'>
				{tiers.map((tier, index) => (
					<div key={index} className='flex flex-col gap-1'>
						...
						{index < tiers.length - 1 && <div className='h-px bg-muted' />}
					</div>
				))}
			</div>
		</div>
		...
		<TooltipContent
			...
			<div className='space-y-3'>
				{isSubscriptionOverride && (
					<div className='space-y-2'>
						<div className='font-medium text-foreground'>{t('priceTooltip.overriddenPrice')}</div>
						{isTiered ? (
				...
				{!isSubscriptionOverride && hasDiscount && discountInfo && (
					<div className='space-y-2'>
						<div className='font-medium text-foreground'>{t('priceTooltip.priceHeading')}</div>
						<div className='space-y-1'>
							...
							{couponName && <div className='text-xs text-muted-foreground mt-1'>{couponName}</div>}
						</div>
					</div>
				)}
				...
				{!isSubscriptionOverride && hasOverrides && !isTiered && originalNormalized && originalPrice && (
					<div className='space-y-2'>
						<div className='font-medium text-foreground'>{t('priceTooltip.priceOverrideApplied')}</div>
