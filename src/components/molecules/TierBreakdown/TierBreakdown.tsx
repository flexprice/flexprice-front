		...
		<Card className='p-4'>
			<div className='space-y-4'>
				<div className='flex justify-between items-center'>
					<h3 className='text-lg font-semibold text-foreground'>{t('tierBreakdown.title')}</h3>
				...
				<div className='grid grid-cols-2 gap-4 text-sm'>
					<div>
						<span className='text-muted-foreground'>{t('tierBreakdown.totalUsage')}</span>
						<span className='ms-2 font-medium'>{usageQuantity}</span>
					</div>
					<div>
						<span className='text-muted-foreground'>{t('tierBreakdown.totalCost')}</span>
				...
				{tiers.length > 0 && (
					<div className='space-y-2'>
						<div className='text-sm font-medium text-foreground'>{t('tierBreakdown.tierDetails')}</div>
						<div className='space-y-1'>
							{tiers.map((tier, index) => (
								<div key={index} className='flex justify-between text-sm py-1 border-b border-border last:border-b-0'>
									<div className='flex-1'>
										<span className='text-muted-foreground'>{t('tierBreakdown.range')}</span>
										<span className='ms-2 font-medium'>{tier.range}</span>
									</div>
									<div className='flex-1'>
										<span className='text-muted-foreground'>{t('tierBreakdown.quantity')}</span>
										<span className='ms-2 font-medium'>{tier.quantity}</span>
									</div>
									<div className='flex-1'>
										<span className='text-muted-foreground'>{t('tierBreakdown.rate')}</span>
										<span className='ms-2 font-medium'>
											{currencySymbol}
											{tier.rate}
										</span>
									...
									<div className='flex-1'>
										<span className='text-muted-foreground'>{t('tierBreakdown.cost')}</span>
										<span className='ms-2 font-medium'>
											{currencySymbol}
											{tier.cost}
										</span>
