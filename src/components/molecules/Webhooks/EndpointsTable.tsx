...
const ErrorRateCell: FC<{ endpointId: string }> = ({ endpointId }) => {
	...
	if (loading || !data) return <span className='text-sm text-muted-foreground'>{t('labels.missingValue')}</span>;
	...
	const columns: ColumnData<EndpointOut>[] = [
		{
			...
			render: (row) => (
				<div className='min-w-0'>
					...
					{row.description && <div className='truncate text-xs text-muted-foreground'>{row.description}</div>}
				</div>
			),
		},
		{
	...
	return (
		<div className='flex flex-col gap-4'>
			...
			{hasEndpoints ? (
				...
				<div className='flex flex-col items-center justify-center rounded-md border border-border bg-muted/50 py-16 px-6 text-center'>
					<div className='mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-card border border-border'>
						<Rss className='h-5 w-5 text-muted-foreground' />
					</div>
					<h3 className='text-base font-medium text-foreground'>{t('webhooks.endpoints.empty.title')}</h3>
					<p className='mt-2 pb-5 max-w-md text-sm text-muted-foreground'>
						<Trans
