				...
				<div className='space-y-6 mt-6'>
					...
					<div className='p-4 bg-muted rounded-lg space-y-3'>
						<div className='flex justify-between items-center'>
							<span className='text-sm text-muted-foreground'>{t('creditNotes.creditNoteType')}</span>
							<Chip
						...
						<div className='flex justify-between items-center'>
							<span className='text-sm text-muted-foreground'>{t('creditNotes.totalAmountLabel')}</span>
				...
				<div className='bg-card border rounded-lg p-6'>
					<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
						<div>
							...
							<div className='text-sm text-muted-foreground'>{t('creditNotes.invoiceNumberLabel')}</div>
						</div>
						<div>
							<div className='text-sm font-medium'>{formatCurrency(invoice?.amount_paid || 0, invoiceCurrency)}</div>
							<div className='text-sm text-muted-foreground'>{t('creditNotes.amountPaid')}</div>
						</div>
						<div>
							<div className='text-sm font-medium'>{formatCurrency(Number(invoice?.amount_remaining), invoiceCurrency)}</div>
							<div className='text-sm text-muted-foreground'>{t('creditNotes.amountRemaining')}</div>
						</div>
					</div>
				</div>

				<div className='flex flex-col gap-4 bg-card border rounded-lg p-6'>
					{/* Reason */}
				...
				<div className='bg-card border rounded-lg divide-y'>
					...
					<div className='p-6'>
						<div className='flex justify-between items-center mb-4'>
							...
							<span className='text-sm text-muted-foreground'>{t('creditNotes.creditAmount')}</span>
						</div>
							...
							{lineItems.map((item) => (
								<div key={item.id} className='flex items-center justify-between'>
									<div className='flex-1'>
										...
										<div className='text-sm text-muted-foreground'>{formatCurrency(item.unit_price, invoiceCurrency)}</div>
									</div>
									<div className='ml-4'>
										<Input
						...
						<div className='flex justify-end'>
							<div className='w-80 space-y-2'>
								...
								<div className='flex justify-between items-center py-1'>
									<span className='text-sm text-muted-foreground'>{t('creditNotes.totalAmountToCredit')}</span>
									<span className='text-sm text-foreground font-medium'>{formatCurrency(totalCreditAmount, invoiceCurrency)}</span>
								</div>
								...
								<div className='flex justify-between items-center py-3 border-t border-border'>
									...
									<span className='text-base font-semibold text-foreground'>{formatCurrency(totalCreditAmount, invoiceCurrency)}</span>
								</div>
							</div>
						</div>
					</div>
