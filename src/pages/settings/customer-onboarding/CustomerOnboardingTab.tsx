			...
			{isLoading ? (
				...
				<>
					<div className='rounded-lg border border-border bg-muted p-4'>
						<p className='text-xs font-medium uppercase tracking-wide text-muted-foreground'>{t('customerOnboarding.workflow.summaryLabel')}</p>
					...
					<div className='mt-6 flex flex-col gap-4 border-t border-border pt-6'>
						<div>
							<p className='text-sm font-medium text-foreground'>{t('customerOnboarding.workflow.customWorkflows.title')}</p>
							<p className='mt-1 text-sm text-muted-foreground'>{t('customerOnboarding.workflow.customWorkflows.description')}</p>
						</div>

						{draft.customWorkflows.length === 0 ? (
							<p className='text-sm text-muted-foreground'>{t('customerOnboarding.workflow.customWorkflows.empty')}</p>
						) : (
